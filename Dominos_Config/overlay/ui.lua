...
function OverlayUI:DrawGrid()
	local hLines = ParentAddon:GetAlignmentGridSize() * 2  --How many times each quadrant off screen is split vertically to create horizontal lines.
	...
	for i = 1, max(hLines, vLines) do --one loop is better than two
		if i <= vLines then
			...
			line:SetStartPoint("BOTTOMLEFT", (gridSize * i) + gridOffset, 0)
			line:SetEndPoint     ("TOPLEFT", (gridSize * i) + gridOffset, 0)
		end
		if i <= hLines then
			line = self:AcquireGridLine()
			line:SetColorTexture(((i == ceil(hLines / 2)) and GRID_HIGHLIGHT_COLOR or GRID_COLOR):GetRGBA())
			line:SetStartPoint("BOTTOMLEFT", 0, gridSize * i)
			line:SetEndPoint ("BOTTOMRIGHT", 0, gridSize * i)
		end
	end
end
